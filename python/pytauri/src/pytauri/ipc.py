...
from inspect import Parameter, Signature, signature, iscoroutinefunction, isawaitable
...
class Commands(UserDict[str, _PyInvokHandleData]):
    ...
    def wrap_pyfunc(  # noqa: C901, PLR0912, PLR0915  # TODO: simplify this method
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> _InvokeResponseBody:
            ...
            if iscoroutinefunction(pyfunc):
                resp = await pyfunc(*args, **kwargs)
            elif isawaitable(pyfunc):
                resp = await pyfunc
            else:
                resp = await to_thread.run_sync(partial(fn, *args, **kwargs))
