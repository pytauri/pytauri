...
async def future_to_coroutine(
    ...
    async def _acallback(future: futures.Future[T]) -> None:
        _callback(future)
    ...
    def callback(future: futures.Future[T]) -> None:
        if get_ident() == event_loop_ident:
            ...
            if isinstance(executor, BlockingPortal):
                ...
                executor.start_soon(_acallback, future)
    ...
    with (
        ...
        async def main() -> None:
            async with (
                ...
                result = await(await future_to_coroutine(future1, tg))
                print(result)

                result = await(await future_to_coroutine(future, blocking_portal))
